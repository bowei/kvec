...
of set containment over plain strings. It lives in \code{pkg/sig} alongside
\code{sig.Fixed}, the compile-time sized vector \code{KV} embeds; \code{kv}
depends on that package rather than the other way round.
...
two \code{sig.Fixed} vectors of \code{sig.FixedBits}${}=128$ bits, built at
construction: \code{ksig}
...
the argument for the inline \code{sig.Fixed} inside \code{KV}: at two words the
...
fills it. A \code{KV} carries \code{sig.FixedBits}${} = 128$ bits, so a set of
...
quantitative form of the note in the source that \code{sig.Fixed} suits sets of
far fewer than a thousand pairs: the gate is worth having up to a hundred or so
pairs, is half wasted by $128$, and is dead weight by $1024$. A set expected to
run large wants a wider \code{sig.Fixed}, at two words of overhead per doubling.
